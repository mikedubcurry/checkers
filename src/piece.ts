...
export class Piece {
	...
	move(newPosition: IPosition, board: Board, attacking: boolean) {
		...
		if (attacking) {
			// delete piece
			let midRank: TRank = Ranks[Math.floor((ERank[newPosition.rank] + ERank[oldPosition.rank]) / 2)];
			let midFile = Files[Math.floor((EFile[newPosition.file] + EFile[oldPosition.file]) / 2)];
	...
	public canMoveTo(newPosition: IPosition, board: Board, attacking: boolean) {
		...
		if (attacking) {
			if (newPosition.file !== file && Math.abs(EFile[newPosition.file] - EFile[file]) === 2) {
				if (newPosition.rank !== rank && Math.abs(ERank[newPosition.rank] - ERank[rank]) === 2) {
			...
			throw new MoveError('cannot move there');
			return false;
		} else {
			if (newPosition.file !== file && Math.abs(EFile[newPosition.file] - EFile[file]) === 1) {
				if (newPosition.rank !== rank && Math.abs(ERank[newPosition.rank] - ERank[rank]) === 1) {
					if (!this.king) {
						if (this.color === 'red') {
